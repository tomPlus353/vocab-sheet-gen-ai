...
- For storage writes, prefer bulk or batched database operations over row-by-row loops when the request can touch many terms or history rows; this helps avoid serverless timeouts on live deployments.
...
- When changes are made to the UI, do not run browser testing yourself. Instead, ask the user to verify in their browser with explicit, step-by-step instructions and a clear expected-result checklist.
...
- Workflow tests that can be run in a browser environment are documented in `.codex/workflows/*.md`; reference them when writing user verification instructions.
